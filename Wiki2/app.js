...
function loadJSON(searchTerm, callback) {
   ...
   xobj.open(
      ...
      searchTerm +
      "&srlimit=12&srprop=snippet&origin=*",
      true
   );
   ...
   loadJSON(searchTerm, function(response) {
      ...
      quotesArr.query.search.forEach(function(el) {
         results.insertAdjacentHTML(
            "beforeend",
            '<li class="entry"><a href="https://en.wikipedia.org/?curid=' +
            el.pageid +
            '" class="entry__link" target="_blank"><h3 class="entry__title">' +
            el.title +
            '</h3><p class="entry__snippet">' +
            el.snippet +
            "</p></a></li>"
         );
      });
